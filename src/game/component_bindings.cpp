...
void register_bindings(sol::state& lua) {
    ...
    lua.new_enum("AttributeEasingType",
        "NONE", AttributeEasingType::NONE,
        "JITTER", AttributeEasingType::JITTER,
        "SAW", AttributeEasingType::SAW,
        "SIN", AttributeEasingType::SIN,
        "RANDOM_STEP", AttributeEasingType::RANDOM_STEP
    );

    lua.new_enum("ComponentState",
                 "DEACTIVATED", ComponentState::DEACTIVATED,
                  "ACTIVATING", ComponentState::ACTIVATING,
                  "ACTIVE", ComponentState::ACTIVE,
                  "DEACTIVATING", ComponentState::DEACTIVATING,
                  "ERROR", ComponentState::ERROR,
                  "DESTROYED", ComponentState::DESTROYED,
                  "BLOCKED", ComponentState::BLOCKED
    );
  
    lua.new_enum("ComponentSize",
                  "S", ComponentSize::S,
                    "M", ComponentSize::M,
                    "L", ComponentSize::L
      );
    lua.new_enum("ComponentMaterial",
                 "ALUMINIUM", ComponentMaterial::ALUMINIUM,
                  "COPPER", ComponentMaterial::COPPER,
                  "STEEL", ComponentMaterial::STEEL,
                  "TITANIUM", ComponentMaterial::TITANIUM,
    ...
    lua.new_usertype<Environment>("Environment",
                                  ...
                                  "minutes", &Environment::minutes
                                  );

    // AttributeEasing struct
    lua.new_usertype<AttributeEasing>("AttributeEasing",
        "easing_type", &AttributeEasing::easing_type,
        "easing_range", &AttributeEasing::easing_range,
        "easing_period", &AttributeEasing::easing_period
    );

    // Attribute class
    lua.new_usertype<Attribute>("Attribute",
        sol::constructors<Attribute(const std::string&, const std::string&, AttributeType, AttributeValue, AttributeEasing)>(),
        "GetTitle", &Attribute::GetTitle,
        "GetDescription", &Attribute::GetDescription,
        "GetType", &Attribute::GetType,
        "SetBaseValue", &Attribute::SetBaseValue,
        "GetBaseValue", &Attribute::GetBaseValue,
        "GetFinalValue", &Attribute::GetFinalValue,
        "SetEasing", &Attribute::SetEasing
    );

    // Metadata struct
    lua.new_usertype<Metadata>("Metadata",
        "id", &Metadata::id,
        "name", &Metadata::name,
  ...
  lua.new_usertype<NetworkInfo>("NetworkInfo",
                                "production", &NetworkInfo::production,
                                "consumption", &NetworkInfo::consumption,
                                "accumulated", &NetworkInfo::accumulated,
                                "accumulated_available", &NetworkInfo::accumulated_available,
                                "battery_count", &NetworkInfo::battery_count
                              );
  lua.new_usertype<Frame>("Frame",
                          "data", &Frame::data,
                          "component_limits", &Frame::component_limits,
                          "components", &Frame::components,
                          "getComponentByType", &Frame::getComponentByType,
                          "getPowerInfo", [](Frame &frame) -> NetworkInfo {
                            auto &info = entt::locator<PowerInfo>::value();
                            for (auto &net : info.networks) {
                              if (std::ranges::find(net.frames, frame.data.id) != net.frames.end()) {
                                return net;
                              }
                            }
                          }
...
std::shared_ptr<Component> create_component_from_lua(sol::state& lua, const std::string& lua_source) {
    ...
    for (const auto& pair : attributes) {
        ...
        if (attr_data["easing"].valid()) {
            easing.easing_type = attr_data["easing"]["type"].get_or(AttributeEasingType::NONE);
    ...
    component->data.attributes["temp"] =
