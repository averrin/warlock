  ...
  useEffect(() => {
    ...
    for (const frame of placedFrames) {
      ...
      node.on("pointerdown", (e: FederatedPointerEvent) => {
        e.stopPropagation();
        draggingFrameIdRef.current = frame.id;
        pendingDropRef.current = { id: frame.id, x: frame._x, y: frame._y };
        selectFrame(frame.id);
      });

      const box = new Graphics();
      box.beginFill(selectedFrameId === frame.id ? 0x2563eb : 0x334155);
      box.roundRect(0, 0, 110, 68, 10);
      box.fill();
      node.addChild(box);
