...
public:
  ...
  Attribute(){}
  Attribute(const Attribute& other): title(other.title), description(other.description), type(other.type), baseValue(other.baseValue) {
  ...
  void SetEasing(AttributeEasing e) {
    easing = e;
    if(easing.easing_type == AttributeEasingType::NONE) return;
    ...
    if (easing.easing_type == AttributeEasingType::SAW) {
      ...
      AddModifier(std::make_shared<Modifier>([](const Attribute &attr) {
  ...
  template <class T> T get() const {
    return std::get<T>(GetFinalValue());
  }
  template <class T> T getBase() const {
    return std::get<T>(GetBaseValue());
  }
