...
class ThermalSystem : public System {
  ...
  std::map<EffectID, std::shared_ptr<Modifier>> effects;

public:
  static constexpr auto heat_effect_temp = 150.0f;
  static constexpr auto cold_effect_temp = -50.0f;
  static constexpr auto max_break_temp = 200.0f;
  static constexpr auto min_break_temp = -100.0f;

  void fixedUpdate() override;
  ThermalSystem() : System(50, "Thermal") {
    ...
    effects[EffectID::OVERHEAT] =
        std::make_shared<Modifier>("Overheat", [](const Attribute &attr) {
          return std::get<float>(attr.GetBaseValue()) * 0.5f;
        });
    effects[EffectID::FREEZE] =
        std::make_shared<Modifier>("Freeze", [](const Attribute &attr) {
          return std::get<float>(attr.GetBaseValue()) * 1.5f;
        });
  }

  std::map<ComponentMaterial, Material *> materials;
  std::map<ComponentSize, float> volumes = {{ComponentSize::S, 0.001},
  ...
  float calculateConduction(Component *c1, Component *c2, float contactArea) {
    ...
    float thermalConductance =
        (m1->thermalConductivity + m2->thermalConductivity) / 2.0;
